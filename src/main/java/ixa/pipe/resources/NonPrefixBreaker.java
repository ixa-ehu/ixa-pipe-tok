...
public class NonPrefixBreaker {
	...
	// Segmenter Patterns 
		...
		public static Pattern NOPERIOD_END = 
				Pattern.compile("([?!])\\s+([\'\"\\(\\[\\¿\\¡\\p{Punct}]*[\\p{Upper}])");
		// multi-dots followed by sentence starters
		public static Pattern MULTI_DOTS_STARTERS = Pattern.compile("(\\.[\\.]+)\\s+([\'\"\\(\\[\\¿\\¡\\p{Punct}]*[\\p{Upper}])");
		...
		public static Pattern END_INSIDE_QUOTES = Pattern.compile("([?!\\.][\\ ]*[\'\"\\)\\]\\p{Punct}]+)\\s+([\'\"\\(\\[\\¿\\¡\\p{Punct}]*[\\ ]*[\\p{Upper}])");
		...
		public static Pattern NOPERIOD_END_SPACE= Pattern.compile("([?!])\\s+([\'\"\\(\\[\\¿\\¡\\p{Punct}]+[\\ ]*[\\p{Upper}])");
		public static Pattern ALPHANUM_PUNCT = Pattern.compile("([\\p{Alnum}\\.\\-]*)([\'\"\\)\\]\\%\\p{Punct}]*)(\\.+)");
		...
		public static Pattern SPECIALS = Pattern.compile("([^\\p{Alnum}\\s\\.\'\\`\\,\\-])");
		public static Pattern MULTI_DOTS = Pattern.compile("\\.([\\.]+)");
		public static Pattern DOT_MULTI = Pattern.compile("DOTMULTI\\.");
		public static Pattern MULTI_MULTI_DOTS = Pattern.compile("DOTMULTI\\.([^\\.])");
		public static Pattern NODIGIT_COMMA_NODIGIT = Pattern.compile("([^\\d])[,]([^\\d])");
		public static Pattern DIGIT_COMMA_NODIGIT = Pattern.compile("([\\d])[,]([^\\d])");
		public static Pattern NODIGIT_COMMA_DIGIT = Pattern.compile("([^\\d])[,](\\d)");
		...
		public static Pattern NOALPHA_APOS_NOALPHA = Pattern.compile("([^a-zA-Z])[']([^a-zA-Z])");
		public static Pattern NOALPHA_DIGIT_APOS_NOALPHA = Pattern.compile("([^A-Za-z]\\d])[\']([a-zA-Z])");
		public static Pattern ALPHA_APOS_NOALPHA = Pattern.compile("([a-zA-Z])[']([^A-Za-z])");
		public static Pattern ALPHA_APOS_ALPHA = Pattern.compile("([a-zA-Z])[']([a-zA-Z])");
		...
		public String SegmenterNonBreaker(String line) {
			...
			for (int i=0; i < words.length; i++) { 
				...
				if (word.matches(STRING_ALPHANUM_PUNCT)) {
					...
					if (word.matches(pre) && dictMap.containsKey(pre) && (dictMap.get(pre) == "1") && !word.matches(startPunct))
					{
						//not breaking
						return word;
					}
					...
					else if (word.matches("^([ ]*[\'\"\\(\\[\\¿\\¡\\p{Punct}]*[ ]*[\\p{Upper}0-9])")) { 
						// the next word has a bunch of initial quotes, maybe a space, 
						// then either upper case or a number
						while (word.matches(pre) && dictMap.containsKey(pre) && (dictMap.get(pre) == "2") && !word.matches(startPunct) && words[i+1].matches("^[0-9]+")) { 
							word = word + "\n";
						}
		...
		/*public String nonPrefixBreaker(String line) {
			...
			for (int i=0; i < words.length; i++) { 
				...
				if (word.matches("^(\\S+)\\.$")) {
					String pre = WORD_DOT.matcher(word).replaceAll("$1");
					...
					else if (this.numericOnly.contains(pre) && (i < (words.length)-1) && 
