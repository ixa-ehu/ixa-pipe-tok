...
public class TokenizerMoses implements TokTokenizer {
  ...
  /*
   * public String treatContractions(String line) { if
   * (lang.equalsIgnoreCase("en")) { line =
   * NonPrefixBreaker.NOALPHA_APOS_NOALPHA.matcher(line).replaceAll("$1 ' $2");
   * line =
   * NonPrefixBreaker.NOALPHA_DIGIT_APOS_NOALPHA.matcher(line).replaceAll(
   * "$1 ' $2"); line =
   * NonPrefixBreaker.ALPHA_APOS_NOALPHA.matcher(line).replaceAll("$1 ' $2");
   * line =
   * NonPrefixBreaker.ALPHA_APOS_ALPHA.matcher(line).replaceAll("$1 '$2"); line
   * = NonPrefixBreaker.YEAR_APOS.matcher(line).replaceAll("$1 ' $2"); } else {
  ...
  public String[] tokenize(String line) {
    ...
    while (multiMatcher.find()) {
      line = NonPrefixBreaker.MULTI_MULTI_DOTS.matcher(line).replaceAll(
          "DOTDOTMULTI $1");
      line = NonPrefixBreaker.DOT_MULTI.matcher(line).replaceAll("DOTDOTMULTI");
    }
    line = NonPrefixBreaker.NODIGIT_COMMA_NODIGIT.matcher(line).replaceAll(
        "$1 , $2");
    line = NonPrefixBreaker.DIGIT_COMMA_NODIGIT.matcher(line).replaceAll(
        "$1 , $2");
    line = NonPrefixBreaker.NODIGIT_COMMA_DIGIT.matcher(line).replaceAll(
        "$1 , $2");
